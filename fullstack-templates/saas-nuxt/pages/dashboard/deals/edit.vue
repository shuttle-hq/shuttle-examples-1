<template>
  <Layout>
    <form class="py-10 flex flex-col gap-4 justify-center items-center" @submit.prevent="handleSubmit">
      ...
      <label for="lastname">
        <span>Last name: </span>
        <input type="email" name="lastname" class="px-5 py-2" v-model="lastName" />
      </label>
      <label for="email">
        <span>Email address: </span>
        <input type="text" name="email" class="px-5 py-2" v-model="email" />
      </label>
      <label for="phone">
